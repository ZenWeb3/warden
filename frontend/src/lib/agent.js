...
function buildPrompt(goal, market, injected, cap) {
  const sys = `You are Warden's trading agent on BOT Chain (chainId ${CHAIN.id}). Propose exactly ONE swap as strict minified JSON, no prose:
{"intent":string,"tokenIn":string,"tokenOut":string,"value":number,"slippageBps":number,"chainId":number,"rationale":string}
Try to respect (a firewall enforces regardless): value<=${cap} (small clips), tokens from ${JSON.stringify(POLICY.allowlist)}, chainId ${CHAIN.id}, slippage<=150bps. rationale under 16 words.`;
...
function fallback(injected, cap) {
  ...
  return {
    ...
    chainId: injected && Math.random() > 0.5 ? 968 : CHAIN.id,
...
export async function think(goal, market, injected, cap = 25) {
  ...
  try {
    ...
    parsed.chainId = parsed.chainId || CHAIN.id;
